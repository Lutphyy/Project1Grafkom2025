...
// Konversi data ke format yang diperlukan (menghitung rata-rata CPMK)
...
// Fungsi menggambar bubble untuk data yang ada pada halaman aktif
function drawBubbles() {
  // Ambil subset data sesuai halaman
  var startIndex = (currentPage - 1) * pageSize;
  var endIndex = startIndex + pageSize;
  var currentPoints = points.slice(startIndex, endIndex);
  ...
  currentPoints.forEach(function(p) {
    ...
    ctx.fillText(p.nim, cx - r, cy - r);
    // Simpan posisi untuk tooltip
    p.cx = cx;
    p.cy = cy;
    p.r = r;
  });
...
// Fungsi utama untuk menggambar chart
function drawChart() {
...
// Event untuk tombol Prev dan Next
